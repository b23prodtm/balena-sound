...
if [[ "$OSTYPE" == "linux"* ]]; then
    # Save current dist-packages path into venv
    printf "export DBUS_SYSTEM_BUS_ADDRESS=%s PYTHONPATH=%s" \
"${DBUS_SYSTEM_BUS_ADDRESS}" "${PYTHONPATH}" | tee -a $TOPDIR/venv/bin/activate
    # Use the venv
